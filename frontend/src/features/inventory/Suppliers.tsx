...
const Suppliers: React.FC = () => {
  ...
  const [creditPaymentForm, setCreditPaymentForm] = useState({
    ...
    branch_id: branchId || '', // Fallback to empty string if null
  });
  ...
  // Memoized to prevent unnecessary re-creations and satisfy ESLint
  const fetchSuppliers = useCallback(async () => {
    ...
    try {
      ...
      const suppliersArray = response.data.suppliers || [];
      
      setSuppliers(suppliersArray);

      
      setGlobalTotalDebt(response.data.metrics?.tenant_total_credit || 0);
    } catch (err) {
      console.error("Failed to load suppliers", err);
      setMessage({ type: 'error', text: 'Failed to load suppliers.' });
      setSuppliers([]); // Defensively set to empty array on error
    } finally {
      setIsLoading(false);
    }
  }, [searchQuery]); // Re-creates only when searchQuery changes
  ...
  const handleOpenPayment = (supplier: Supplier) => {
    ...
    // FIX: Match pre-filled amount to user's permission scope
    const relevantDebt = isAdmin ? supplier.current_debt : supplier.branch_specific_debt;
  ...
  const handleProcessPayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // FIX: Proper type narrowing for Axios errors
      if (isAxiosError(err)) {
    ...
    <div className="max-w-7xl mx-auto p-4 space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-200 gap-4">
        ...
        <div className="bg-red-50 p-6 rounded-xl shadow-sm border border-red-100 flex flex-col justify-center items-center text-center">
            <div className="text-sm font-bold text-red-600 uppercase tracking-wider mb-1">Total Accounts Payable</div>
            ...
            <table className="min-w-full divide-y divide-gray-200 text-left">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {suppliers.map((supplier) => {
                  // FIX: Standard JS block comments instead of JSX block comments outside the return payload
                  /*
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in-down">
            ...
            <div className="p-5 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
              ...
              <span className="text-xl font-extrabold text-red-600">₦{Number(isAdmin ? paymentSupplier.current_debt : paymentSupplier.branch_specific_debt).toLocaleString()}</span>
            </div>
            ...
            <form onSubmit={handleProcessPayment} className="p-5 space-y-4">
              ...
              <div>
                ...
                <input 
                  ...
                  required 
                  min="1"
                  max={isAdmin ? paymentSupplier.current_debt : paymentSupplier.branch_specific_debt}
